...
for(i in 1:length(target.metadata)){
  ...
  p.ca.assymetrical <- ggplot(plot.row.principal, aes(x = PC1, y = PC2, color = type, label = label)) +
    geom_point(show.legend = T) + coord_cartesian(xlim = c(-5,5), ylim = c(-5,5)) +
